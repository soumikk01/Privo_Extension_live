...
// structural delimiter tag names that appear in the assembled prompt. A malicious
// page cannot break out of <browser_state> or forge an authoritative <user_request>
// by embedding these tag names in visible text, titles, or element labels.
//
// Tag inventory (matched by this regex):
//   browser_state   — the section this content is embedded in
//   user_request    — highest-priority section ("make the user happy")
//   instructions, system_instructions, page_instructions
//                   — authoritative instruction sections
//   agent_history, agent_state, step_info, step_\d+, sys
//                   — structural history/state sections
//   llms_txt        — llms.txt-derived context section
//
// NOTE: `user_task` was a stale entry — the actual tag is `user_request`.
//       `system\b` had a word-boundary gap with `system_instructions`
//       (because `_` is a word char). Both are fixed here.
